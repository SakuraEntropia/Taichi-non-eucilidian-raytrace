...
@ti.func
def shape_function(t,o,dir) -> vec3:
    distance = (o + t * dir - center).norm()
    return 0.5 * ((ti.tanh((distance + L ) * w) - ti.tanh((distance - L) * w)) / ti.tanh(L * w))

@ti.func
def rk4_integrate(f: ti.template(), y0: vec3, t0: ti.f32, t1: ti.f32, dir:vec3, dt: ti.f32) -> vec3:
    """
    Integrates the given function using the fourth-order Runge-Kutta method.

    Args:
        f: The function to integrate.
        y0: The initial value of the function.
        t0: The initial time.
        t1: The final time.
        dt: The time step.
    ...
    while t < t1:
        k1 = f(t, y ,dir)
        k2 = f(t + dt / 2, y + dt * k1 / 2,dir)
        k3 = f(t + dt / 2, y + dt * k2 / 2,dir)
        k4 = f(t + dt, y + dt * k3,dir)
...
objects[3] = Object(type=SHAPE_SPHERE,
                    trs=Transform(vec3(-0.5, -0.2, 0), vec3(0.3)),
...
def raycast(ray) -> HitRecord:  # 光线步进求交
    ...
    for _ in range(MAX_RAYMARCH):   # 光线步进
        ...
        SF[1]=rk4_integrate(shape_function,ray.origin,0,dis,ray.direction,0.1)
        record.position = ray.at(record.distance) + vec3((SF[1].x * 100),0,0)
          # 计算光子所在位置
