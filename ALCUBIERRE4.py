...
PRECISION   = 0.01                # 必须要小于 TMIN，否则光线会自相交产生阴影痤疮
...
SF = ti.Vector.field(n=3,dtype=float,shape=(1,))


@ti.func
def shape_function(r) -> vec3:
    distance = (r - center).norm()
    return 0.5 * ((ti.tanh((distance + L ) * w) - ti.tanh((distance - L) * w)) / ti.tanh(L * w))
...
@ti.func
def integrate(a: vec3, b: vec3, n: ti.i32) -> vec3:
    # Integration logic (based on chosen method)
    result = ti.Vector([0.0, 0.0, 0.0])  # Adjust dimensions as needed
    dx = (b - a) / n
    for i in range(n):
        x = a + i * dx
        result += shape_function(x) * dx  # Example for Riemann sum
    return result

a = 0.0
b = 1.0
n = 1000  # Adjust number of intervals

@ti.func
def raycast(ray) -> HitRecord:  # 光线步进求交
    record = HitRecord(ray.origin, TMIN, False)
    SF[1]=(0,0,0)# 初始化光子碰撞记录
    for _ in range(MAX_RAYMARCH):   # 光线步进
        record.obj = nearest_object(record.position)   # 计算光子与球体的有向距离
        # 绝对值为无符号距离
        dis = abs(record.obj.sd)# 绝对值为无符号距离
        SF[1] = integrate(ray.origin,ray.origin+ray.direction*dis,300)*100
        record.position = ray.at(record.distance) + vec3((SF[1].x ),0,0)
          # 计算光子所在位置
        if dis < PRECISION: # 如果光子与球体的距离小于精度即为击中
            record.hit = True   # 设置击中状态
            break
        record.distance += dx  # 光子继续传播
